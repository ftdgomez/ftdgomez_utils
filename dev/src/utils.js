 ...
 * @returns undefined
 */
export function _handleError(res, error) {
  res
 ...
 */
export async function _fetch(url, method, body, addHeaders) {
  const urlBase =
 ...
 * }
 */
export function _setArrayToObject(array, key) {
  let temp = {};
  Object.assign(
    temp,
    ...array.map((o) => {
 ...
 * @returns Date + Number
 */
export function _daysFromDate(date, days = 0) {
 ...
 */
export function _checkAuthorizationCookie(context, from) {
 ...
 */
export function _objectHasValue(obj, properties) {
  try {
 ...
 */
export function _objectSize(obj) {
 ...
 */
export function _where(array, properties) {
  let response = [];
  array.forEach((el) => {
